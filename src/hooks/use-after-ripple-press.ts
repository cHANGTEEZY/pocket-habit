...
type UseAfterRipplePressOptions = {
  /**
   * Wait this long so ripple/scale can finish before running the action
   * (avoids mid-animation style swaps from parent re-renders).
   */
  delayMs?: number;
  /** When false, presses are ignored. */
  enabled?: boolean;
  /** Run the action after InteractionManager settles (default true). */
  afterInteractions?: boolean;
};

/**
 * Defers a press action until after a press ripple has had time to finish.
 * Use with PressableFeedback.Ripple `progress.baseDuration` set to the same
 * `rippleSettleMs` value.
 */
export function useAfterRipplePress(
  ...
  return {
    onPress,
    /** Pass to PressableFeedback.Ripple `progress.baseDuration`. */
